...
import de.bauersoft.data.entities.Role;
...
public class UsersView  extends Div {
	...
	public UsersView(UserService userService, UserDataProvider dataProvider, PasswordEncoder encoder) {
		...
		grid.addItemDoubleClickListener(
				event -> createDialog(userService, dataProvider, event.getItem(), DialogState.EDIT));
		
		GridContextMenu<User> contextMenu = grid.addContextMenu();
		contextMenu.addItem("new", event -> createDialog(userService, dataProvider, new User(), DialogState.NEW));
	...
	private Dialog createDialog(UserService userService, UserDataProvider dataProvider, User item, DialogState state) {
		...
		binder.forField(passwordField).bind((user) -> DialogState.NEW.equals(state) ? "" : "123456",
				(user, value) -> user.setPassword(value != "123456" ? encoder.encode(value) : user.getPassword()));
		...
		binder.forField(confirmPasswordField)
				...
				.bind((user) -> DialogState.NEW.equals(state) ? "" : "123456", (user, value) -> user
						.setPassword(value != "123456" ? encoder.encode(value) : user.getPassword()));
